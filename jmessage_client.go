...
func doReadAndSendMessage(recipient string, messageBody string, globalPriv1 PrivKeyStruct) error {
	...
	// Printing ciphertext for the attacker
	// fmt.Println("Cipher Text", string(encryptedMessage))
...
func performPaddingOracleAttack(ciphertext CiphertextStruct, victimUsername string, username string, privKey PrivKeyStruct) string {
	...
	//modifiedC2Bytes has no delimeter 0x3A
	// Encode the modified ciphertext back to base64
	// modifiedC2 := base64.StdEncoding.EncodeToString(modifiedC2Bytes)
	...
	// err = sendMessageToServer(username, victimUsername, jsonMessage, 0)
	// if err != nil {
	// 	fmt.Printf("Failed to send message to Alice: %v\n", err)

	// }

	// Iterate over each character in the ciphertext starting from len(victimUsername2) + 1 (skip username and delimiter)
	// for i := 0; i < ciphertextLength-4-len(victimUsername2)-1; i++ {
	// 	// Create a copy of the modified ciphertext
	// 	modifiedCiphertext := make([]byte, ciphertextLength)
	// 	copy(modifiedCiphertext, modifiedC2Bytes)

	// 	// Bruteforce the current character by XORing with 2^8 bits
	// 	for j := 0; j < 128; j++ {
	// 		copy(modifiedCiphertext, modifiedC2Bytes)
	// 		modifiedCiphertext[len(victimUsername2)+1+i] ^= byte(j)

	// 		// Calculate the CRC32 checksum of the modified ciphertext
	// 		// Calculate CRC(A)
	// 		crc32Original := binary.BigEndian.Uint32(modifiedC2Bytes[ciphertextLength-4:])

	// 		// Calculate CRC(B) ****
	// 		crcB := make([]byte, ciphertextLength-4)
	// 		for k := 0; k < len(crcB); k++ {
	// 			crcB[k] = 0x00
	// 		}
	// 		crcB[len(victimUsername2)+1+i] = byte(j)
	// 		crc32B := crc32.ChecksumIEEE(crcB)
	// 		fmt.Println(crcB)

	// 		// Calculate CRC(C) = crc(0)
	// 		crcC := make([]byte, ciphertextLength-4)
	// 		for k := 0; k < ciphertextLength-4; k++ {
	// 			crcC[k] = 0x00
	// 		}
	// 		crc32C := crc32.ChecksumIEEE(crcC)

	// 		// Calculate the new CRC32 checksum
	// 		crc32Checksum := crc32Original ^ crc32B ^ crc32C
	// 		binary.BigEndian.PutUint32(modifiedCiphertext[ciphertextLength-4:], crc32Checksum)
	...
	// 		// Check if a read receipt was received from Alice
	// 		messageList, err := getMessagesFromServer(privKey)
	// 		if err != nil {
	// 			fmt.Printf("Failed to retrieve messages: %v\n", err)
	// 			continue
	// 		}
...
func attackMessenger(ciphertextFilename string, victimUsername string, privKey PrivKeyStruct) {
	...
	// plaintext := performPaddingOracleAttack(rawMessage, victimUsername, privKey)
	...
	plaintext := performPaddingOracleAttack(rawMessage, victimUsername, username, privKey)
	fmt.Println("Found PLaintext: ",plaintext)
	// modified_username := "bharlie"
	// modifiedc2 := modifyUsernameCiphertext(c2Bytes, modified_username)

	// Modify the JSON sig part and sign it using Mallory's private signing key
	// modifiedMessage := rawMessage
	// modifiedMessage.C2 = base64.StdEncoding.EncodeToString(modifiedc2)
	// modifiedMessage.Sig = signMessage(modifiedMessage, privKey)
	// //
	// jsonMessage, err := json.Marshal(modifiedMessage)
	// if err != nil {
	// 	fmt.Printf("Failed to marshal modified message: %v\n", err)
	// 	return
	// }

	//Send this msg to alice
	// err = sendMessageToServer(username, victimUsername, jsonMessage, 0)
	// if err != nil {
	// 	fmt.Printf("Failed to send message to Alice: %v\n", err)
	// 	return
	...
	// fmt.Printf("Message sent to Alice successfully.\n")
